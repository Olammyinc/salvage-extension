...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.type) {
    ...
    case 'resume-scan':
      // Popup-initiated wake: the popup observed a persisted SCANNING checkpoint
      // and is requesting the worker to resume processing. Safe if the scan is
      // already DONE/FAILED/IDLE — resume() is a no-op for non-SCANNING phases.
      // Cannot restart or clear a scan; it only continues an incomplete one.
      // Propagate a truthful {ok:false} when resume returns a failed controller
      // outcome (e.g. all storageSet writes rejected), keeping the trusted-sender
      // and normal idempotent {ok:true} behavior otherwise.
      handleResume()
        .then((res) => {
          if (res && res.failed) {
            sendResponse({ ok: false, phase: res.phase || PHASE.FAILED, error: res.error || 'resume failed' });
          } else {

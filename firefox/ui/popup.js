...
(function (root) {
  ...
  // One-shot resume-scan wake. When the popup opens and observes a persisted
  // SCANNING checkpoint, it sends a single resume-scan message to wake the
  // worker. Exactly one send per popup lifecycle (per open) — the flag is
  // never reset while the popup lives, so storage-change events during the
  // same popup session never trigger a duplicate wake.
  let resumeScanSent = false;

  /**
   * Send a one-shot resume-scan message to wake the worker when the popup
   * observes a persisted SCANNING checkpoint. Exactly one send per popup
   * lifecycle — subsequent calls (including from storage-change refreshes)
   * are no-ops.
   */
  function sendResumeScan() {
    if (resumeScanSent) { return; }
    const cp = snapshot.checkpoint;
    if (!cp || cp.phase !== PHASE.SCANNING) { return; }
    resumeScanSent = true;
    chrome.runtime.sendMessage({ type: 'resume-scan' }, () => {
      // Best-effort wake; ignore response. Swallow chrome.runtime.lastError
      // to prevent MV3 "unchecked runtime.lastError" warnings.
  ...
  function renderReport() {
    ...
    // Derive the live duplicate count from the current persisted records (which
    // exclude soft-deleted entries), NOT from the scan-time report snapshot. After
    // a Trash move/restore/undo the records carry updated `deletedAt` markers but
    // the report is never regenerated, so the report's DUPLICATES count is stale.
    // Using the cleanup module's computeDuplicateGroups keeps the visible metric
    // consistent with the duplicates list (which already derives from records).
    const liveDuplicateCount = root.BRCleanup.computeDuplicateGroups(snapshot.records || []).totalDuplicates;

    const rows = [
      { key: 'duplicates', count: liveDuplicateCount, label: COPY.duplicatesLine(liveDuplicateCount), cta: COPY.duplicatesCta },
  ...
  function buildList(records, key) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (key === 'newFolder') {
        ...
        // Guard: snapshot.report may be null after a rescan while a stale list
        // is active (the report is dropped at scan start and only repopulated
        // at completion). Without this guard, accessing GENERATED_AT on null
        // throws. Treat a missing report as "no stale items" — the list
        // renders empty/read-only until the rescan finishes.
        const generatedAt = snapshot.report ? snapshot.report[METRIC.GENERATED_AT] : null;
        if (generatedAt !== null && root.BRReport.isStaleOverYears(r, generatedAt)) { list.push(r); }
  ...
  function openList(key, count) {
    ...
    // Default read-only routes: newFolder, stale, noRecordedOpening. These are
    // informational lists that never offer cleanup selection. Clear any stale
    // cleanup selection/state left by an earlier cleanup-capable list (duplicates
    // or unreachable) so no checkbox or Move-to-Trash action remains visible.
    cleanupSelectionAfterAction();
    const records = snapshot.records || [];
  ...
  function refreshFromStorage() {
    chrome.storage.local.get([KEYS.CHECKPOINT, KEYS.REPORT, KEYS.RECORDS, KEYS.LINK_REPORT, KEYS.LINK_CHECKPOINT], (res) => {
      ...
      // If the popup refreshed and the checkpoint is SCANNING, attempt a
      // resume-scan wake. sendResumeScan is one-shot per popup lifecycle so
      // storage-change events during the same session never send a duplicate.
  ...
  function init() {
    loadSnapshot().then(() => {
      ...
      if (cp && cp.phase === PHASE.SCANNING) {
        renderProgress(cp);
        renderLinkCheck();
        // Wake the worker if it's not running. The persisted SCANNING checkpoint
        // may be stale (worker terminated, no alarm pending); sending a single
        // resume-scan message ensures the controller picks up from the persisted
        // cursor. One-shot: exactly one send per popup lifecycle.
        sendResumeScan();
      } else {
        renderOrEmpty();
      }
